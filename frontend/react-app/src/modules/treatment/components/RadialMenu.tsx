...
const VIEWPORT_MARGIN = 12; // Aire mínimo entre el menú y el borde de la ventana
const TOTAL_SIZE = SUB_RADIUS * 2 + 20; // Diámetro del menú con sub-items desplegados
...
const RadialMenu: React.FC<RadialMenuProps> = ({
  ...
  // Posición efectiva: el centro deseado (x, y) ajustado para que el menú
  // completo quepa en la ventana, y escala para pantallas donde no cabe.
  ...
  useLayoutEffect(() => {
    function place() {
      ...
      // En tablet/móvil el menú puede ser más ancho que la ventana: lo
      // reducimos hasta que quepa en el eje más corto antes de posicionarlo.
      const available = Math.min(vw, vh) - VIEWPORT_MARGIN * 2;
      const scale = Math.min(1, available / TOTAL_SIZE);
      const half = (TOTAL_SIZE * scale) / 2;

      // Con la escala aplicada el rango siempre es válido (min <= max).
      const clamp = (value: number, viewport: number) =>
        Math.min(
          Math.max(value, half + VIEWPORT_MARGIN),
          viewport - half - VIEWPORT_MARGIN,
        );
  ...
  useEffect(() => {
    ...
    // iPad/iOS no siempre sintetiza mousedown fuera de elementos interactivos.
  ...
  const handleMainClick = useCallback(
    (item: RadialMenuItem, e: React.MouseEvent) => {
      e.stopPropagation();
  ...
  const handleSubClick = useCallback(
    (sub: RadialMenuSubItem, e: React.MouseEvent) => {
      e.stopPropagation();
  ...
  return createPortal(
    <div
      ...
      style={{
        ...
        top: placement.top,
        position: "fixed",
        transform: `translate(-50%, -50%) scale(${placement.scale * (visible ? 1 : 0.94)})`,
        zIndex: 9999,
      }}
    >
      ...
      >
        <g className="odn-radial-sectors">
          {items.map((item, i) => {
            ...
            const iconRadius =
              INNER_RADIUS + (OUTER_RADIUS - INNER_RADIUS) * 0.35; // Icono más adentro
            const iconX = Math.cos(midAngle) * iconRadius;
            const iconY = Math.sin(midAngle) * iconRadius;

            const labelRadius =
              INNER_RADIUS + (OUTER_RADIUS - INNER_RADIUS) * 0.75; // Label más afuera
            ...
            return (
              <g
                key={item.id}
                className="odn-radial-sector-group"
              ...
              >
                ...
                <circle
                  ...
                  cy={iconY}
                  r="16"
                  fill={item.id === "borrar" ? "#ef4444" : "#10b981"}
                />
                <text
                  x={iconX}
                  y={iconY + 4}
                ...
                {item.subItems && item.subItems.length > 0 && (
                  <path
                    d={`M ${labelX - 4} ${labelY + 12} L ${labelX + 4} ${labelY + 12} L ${labelX} ${labelY + 16} Z`}
                  ...
                  <g className="odn-radial-sub-sectors">
                    {item.subItems.map((sub, si) => {
                      ...
                      return (
                        <g
                          key={sub.id}
                          className="odn-radial-sub-group"
