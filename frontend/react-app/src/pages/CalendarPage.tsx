...
const TIMELINE_COLORS_LIGHT: Record<
  string,
  { bg: string; border: string; text: string }
> = {
  scheduled: { bg: "#dbeafe", border: "#3b82f6", text: "#1e40af" },
  confirmed: { bg: "#CCFBF1", border: "#0D9488", text: "#0F766E" },
  in_progress: { bg: "#fef3c7", border: "#f59e0b", text: "#92400e" },
  completed: { bg: "#f1f5f9", border: "#6B7280", text: "#475569" },
  cancelled: { bg: "#fee2e2", border: "#ef4444", text: "#991b1b" },
};
const TIMELINE_COLORS_DARK: Record<
  string,
  { bg: string; border: string; text: string }
> = {
  scheduled: {
...
export function CalendarPage({
      ...
      clinicalApi
        ...
        .then((res) => {
          const docs = (res.items ?? []).filter(
            (u: any) => u.role === "doctor" && u.status === "active",
          );
  ...
  const weekDays = useMemo(
    () => (viewType === "week" ? getWeekDays(anchor) : [anchor]),
    [anchor, viewType],
  );
  ...
  useEffect(() => {
    loadCalendar();
  }, [anchor, viewType, effectiveDoctorId]);
  ...
  async function loadCalendar() {
    ...
    try {
      ...
      (patientsRes.items || []).forEach((p: any) =>
  ...
  async function onCreate() {
    if (!selectedPatient?.id) {
      notify.error("Selecciona un paciente");
      return;
    }
    setCreating(true);
    try {
    ...
    return (
      <div className="gcal-month-wrap">
        ...
        <div className="gcal-weekday-row">
          {DAY_NAMES.map((d) => (
            <div key={d} className="gcal-weekday-header">
              {d}
            </div>
          ))}
        </div>
      ...
      >
        <div
          style={{
            ...
            minWidth: viewType === "week" ? 600 : 300,
          }}
        >
          {/* Header */}
          <div
              ...
              <>
                ...
                {weekDays.map((day, di) => {
                  ...
                  const dayAppts = appointments.filter((a) => {
                    ...
                    return (
                      toLocalDateString(d) === dayStr && d.getHours() === hour
                    );
                  });
                  return (
                    <div
                    ...
                    >
                      {dayAppts.map((appt) => {
                        ...
                        const endMin = timeToMinutes(
                          appt.endAt || appt.startAt,
                        );
                        const top =
                          ((startMin - HOUR_START * 60) / 60) * SLOT_HEIGHT -
                          h * SLOT_HEIGHT;
                        const height =
                          (Math.max(endMin - startMin, 30) / 60) * SLOT_HEIGHT;
                        const colors =
                          TIMELINE_COLORS[appt.status] ||
                        ...
                        return (
                          <div
                            ...
                            style={{
                              ...
                              left: 2,
                              right: 2,
                              height: Math.min(height, SLOT_HEIGHT - 2),
      ...
      <div className="gcal-header">
        <div className="gcal-header-left">
          ...
          <div className="gcal-nav">
            <button type="button" className="gcal-btn-nav" onClick={prevPeriod}>
              <ChevronLeft size={16} />
            </button>
            <button type="button" className="gcal-btn-nav" onClick={nextPeriod}>
        ...
        <div className="gcal-header-right">
          {canWriteAppointments(session) && (
            <button
              type="button"
              className="gcal-btn-create"
              onClick={() => {
                setCreateDate(toLocalDateString(new Date()));
                setCreateTime("09:00");
                setShowCreate(true);
              }}
            >
          ...
          <div className="gcal-view-switch">
            {(["month", "week", "day"] as ViewType[]).map((v) => (
              <button
                key={v}
                type="button"
                className={`gcal-view-btn${viewType === v ? " gcal-view-btn--active" : ""}`}
      ...
      {selectedAppt && (
        <Modal onClose={() => setSelectedAppt(null)}>
          ...
          {(() => {
            ...
            const colors =
              TIMELINE_COLORS[appt.status] || TIMELINE_COLORS.scheduled;
            ...
            return (
              <div>
                ...
                <div style={{ marginBottom: 8 }}>
                  <strong>Fecha:</strong> {fmtDt(start)}
                </div>
                <div style={{ marginBottom: 8 }}>
                  <strong>Hora:</strong> {fmt(start)}
                  {end ? ` – ${fmt(end)}` : ""}
                </div>
                {appt.reason && (
                  <div style={{ marginBottom: 8 }}>
                ...
                {appt.paymentAmount != null && appt.paymentAmount > 0 && (
                  <div style={{ marginBottom: 8 }}>
                    <strong>Monto:</strong> ${appt.paymentAmount.toFixed(2)} ·{" "}
                    {appt.paymentMethod || "—"}
                  </div>
                )}
              </div>
