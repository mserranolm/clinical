...
export function AppointmentsPage({
  ...
  const hasCreateTimeSlots = TIME_SLOTS.some(
    (slot) => !isCreateDateToday || !isPastDateTime(createDate, slot),
  );
  ...
  useEffect(() => {
    if (canSelectDoctor && session.orgId) {
      clinicalApi
        .listOrgUsers(session.orgId, token)
        .then((res) => {
          const docs = (res.items ?? []).filter(
            (u) => u.role === "doctor" && u.status === "active",
          );
          const list: { id: string; name: string }[] = docs.map((u) => ({
            id: u.id,
            name: u.name,
          }));
          if (
            isAdmin &&
            session.userId &&
            !list.find((d) => d.id === session.userId)
          ) {
            list.unshift({
              id: session.userId,
              name: `${session.name || "Administrador"} (yo)`,
            });
          }
  ...
  async function onCreate(e: FormEvent<HTMLFormElement>) {
    ...
    if (canSelectDoctor && !selectedDoctorId) {
      notify.error(
        ...
        "Debes asignar un doctor a la cita.",
      );
      return;
    }
    if (!selectedPatient?.id) {
      notify.error(
        ...
        "Debes elegir un paciente antes de confirmar el espacio.",
      );
      return;
    }
    ...
    if (!selectedDate || !selectedTime) {
      notify.error(
        ...
        "Selecciona fecha y hora de inicio para agendar.",
      );
      return;
    }
    if (isPastDateTime(selectedDate, selectedTime)) {
      notify.error(
        "Hora no válida",
        "La cita debe ser a futuro. Si es hoy, selecciona una hora igual o posterior a la actual.",
      );
      return;
    }
  ...
  const rowsByDate: Record<string, AppointmentRow[]> = visibleDates.reduce(
    (acc, localDate) => {
      acc[localDate] = [];
      return acc;
    },
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
        ...
        {canWriteAppointments(session) && isCompleted(row.status) && (
          <button
            ...
            style={{
              background: row.paymentPaid ? "#d1fae5" : "#fef3c7",
              color: row.paymentPaid ? "#065f46" : "#92400e",
              borderColor: row.paymentPaid ? "#6ee7b7" : "#fcd34d",
              opacity: row.paymentPaid ? 0.7 : 1,
              cursor: row.paymentPaid ? "default" : "pointer",
            }}
        ...
        {canManageTreatments(session) &&
          ...
          row.status !== "cancelled" && (
            <button
              ...
              onClick={() =>
                isConfirmed(row.status) || row.status === "in_progress"
                  ? goToTreatment(row)
                  : notify.error(
                      "Cita no confirmada",
    ...
    const promise = Promise.all([
      Promise.all(
        requestedDates.map((day) =>
          clinicalApi.listAppointments(did, day, token),
        ),
      ),
      clinicalApi.listPatients("", token),
    ]).then(([appointmentsByDate, patients]) => {
      const allAppointments = appointmentsByDate.flatMap(
        (appointmentPage) => appointmentPage.items || [],
      );
  ...
  async function onResend(id: string) {
    const promise = clinicalApi
      .resendAppointmentConfirmation(id, token)
      .then((r) => {
        loadAppointments(date, effectiveDoctorId);
        return r;
      });
  ...
  async function onCancel(id: string) {
    ...
    const promise = clinicalApi.updateAppointment(
      id,
      { status: "cancelled" },
      token,
    );
  ...
  async function savePayment() {
    ...
    try {
      await clinicalApi.registerPayment(
        payRow.id,
        { paid: payPaid, paymentMethod: payMethod, paymentAmount: payAmount },
        token,
      );
      notify.success(
        payPaid
          ? "Pago registrado — cita finalizada"
  ...
  return (
    <section className="page-section">
      <PageHeader
        ...
        title="Agenda Médica"
        subtitle="Gestión de citas del consultorio"
        action={
          canWriteAppointments(session) ? (
            <button
              type="button"
      ...
      {payRow && (
        <Modal onClose={() => setPayRow(null)}>
          ...
          <p
            style={{ color: "#64748b", fontSize: "0.85rem", marginBottom: 20 }}
          >
          ...
          <form className="card-form" onSubmit={onCreate}>
              ...
              <div className="input-group">
                ...
                <select name="time" required>
                  ...
                  {TIME_SLOTS.map((slot) => (
                    <option
                      ...
                      value={slot}
                      disabled={
                        isCreateDateToday && isPastDateTime(createDate, slot)
                      }
                    >
                ...
                {isCreateDateToday && !hasCreateTimeSlots && (
                  <small style={{ color: "#b45309", fontSize: "0.75rem" }}>
                    Ya no hay horarios disponibles para hoy. Selecciona una
                    fecha futura.
                  </small>
                )}
              </div>
            ...
            <div className="input-group">
              ...
              <textarea
                name="reason"
                rows={2}
                placeholder="Ej: Limpieza dental, revisión de ortodoncia..."
                style={{
                  width: "100%",
                  resize: "vertical",
                  padding: "8px",
          ...
          <div className="card-form">
            <div className="input-group">
              ...
              <div className="inline-actions">
                ...
                <button
                  type="button"
                  onClick={() => loadAppointments(date, effectiveDoctorId)}
                >
                  Actualizar
                </button>
              </div>
      ...
      <article className="card elite-card" style={{ marginTop: 24 }}>
        ...
        >
          <h3 style={{ margin: 0 }}>
            {viewMode === "day"
              ? "Calendario Diario de Atención"
              : viewMode === "week"
                ? "Calendario Semanal de Atención"
                : "Calendario Mensual de Atención"}
          </h3>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div
              className="agenda-view-switch"
              role="tablist"
              aria-label="Modo de visualización de agenda"
            >
            ...
            <div
              className="agenda-view-switch"
              role="tablist"
              aria-label="Formato de visualización de agenda"
            >
        ...
        {displayMode === "list" ? (
              ...
              <tbody>
                {visibleRows.map((row) => (
                  <tr key={row.id}>
                    ...
                    {viewMode !== "day" && (
                      <td>{formatAgendaDate(row.startAt)}</td>
                    )}
                    ...
                    <td>
                      {row.consentSummary && row.consentSummary.total > 0 ? (
                        row.consentSummary.accepted >=
                        row.consentSummary.total ? (
                          <span
                            className="badge status-confirmed"
                            title="Todos firmados"
                          >
                            Completo
                          </span>
                        ) : (
                          <span
                            ...
                            title={`${row.consentSummary.accepted}/${row.consentSummary.total} enlaces.`}
                          >
                            Pendiente
                          </span>
                        )
                      ...
                      <EmptyState
                        ...
                        description={
                          hideCompletedCancelled
                            ...
                            : "Agenda una nueva cita usando el formulario de arriba."
                        }
                      />
                    </td>
                  </tr>
          ...
          <div className="agenda-calendar-wrap">
            <div
              className={`agenda-calendar-grid ${viewMode === "day" ? "is-day" : ""}`}
            >
                ...
                return (
                  <section className="agenda-calendar-column" key={localDate}>
                    <header className="agenda-calendar-column-header">
                      ...
                      <small>
                        {dateRows.length} cita{dateRows.length === 1 ? "" : "s"}
                      </small>
                    </header>
                    <div className="agenda-calendar-column-body">
                      {dateRows.length === 0 ? (
                        ...
                        dateRows.map((row) => (
                          <article
                            key={row.id}
                            className="agenda-calendar-item"
                          >
